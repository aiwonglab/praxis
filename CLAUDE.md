...
Always use a descriptive merge message with the version tag:
```bash
git checkout prod
git merge dev -m "feat: short summary of what shipped (vX.Y.Z.W)"
git push
git checkout dev
```
Never use the default merge message. The merge commit is the ship record —
it should read like a release note, not "Merge branch 'dev' into prod".
